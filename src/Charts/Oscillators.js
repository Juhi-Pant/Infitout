...
const generateChartData = () => {
    ...
    for (let i = 1; i <= totalBars; i++) {
      ...
      const positionInGradient = (i - 1) / (totalBars - 1);
  

      const red = Math.round(startColor[0] + (endColor[0] - startColor[0]) * positionInGradient);
      const green = Math.round(startColor[1] + (endColor[1] - startColor[1]) * positionInGradient); // Should remain 0 if pure red to blue
      const blue = Math.round(startColor[2] + (endColor[2] - startColor[2]) * positionInGradient);
